...
use crate::{
    ...
    resume_workspace::{
        self, DiagnoseResumeRequest, ResumeModuleCandidate, ResumeWorkspaceDraft,
        RewriteResumeModuleRequest,
    },
    resume_workspaces::{
        self, CreateResumeWorkspaceRequest, ResumeWorkspaceJobStatus, ResumeWorkspaceState,
    },
    settings, worker,
};

fn spawn_worker_event(
    app: &tauri::AppHandle,
    app_data_dir: &Path,
    command: &CommandIn,
) -> Result<Value, String> {
    ...
    for line in reader.lines() {
        ...
        let event: EventOut =
            serde_json::from_str(&line).map_err(|e| format!("parse worker event: {e}"))?;
        match event {
...
fn diagnose_resume_workspace_blocking(
    app: tauri::AppHandle,
    request: DiagnoseResumeRequest,
) -> Result<resume_workspace::ResumeDiagnosis, String> {
    let app_data_dir = paths::resolve_data_dir(&app)?;
    let resume_text =
        resume_text::resolve_resume_text(&request.resume_text, request.resume_files.as_deref())?;
...
fn export_resume_workspace_pdf_blocking(
    ...
    let html = resume_pdf_template::build_resume_template_html(
        &draft,
        title.as_deref().unwrap_or("修改后的简历"),
    );
    ...
    if let Some(stdin) = child.stdin.as_mut() {
        stdin
            ...
            .map_err(|e| format!("写入 PDF HTML 失败：{e}"))?;
    }

    let output = child
        .wait_with_output()
        .map_err(|e| format!("等待 PDF 导出失败：{e}"))?;
...
#[tauri::command]
pub fn get_resume_workspace_status_for_job(
    app: tauri::AppHandle,
    encrypt_job_id: String,
) -> Result<Option<ResumeWorkspaceJobStatus>, String> {
    let app_data_dir = paths::resolve_data_dir(&app)?;
    resume_workspaces::get_status_for_job(&app_data_dir, &encrypt_job_id)
}
...
#[tauri::command]
pub fn switch_resume_workspace(
    app: tauri::AppHandle,
    workspace_id: String,
) -> Result<ResumeWorkspaceState, String> {
    let app_data_dir = paths::resolve_data_dir(&app)?;
...
#[tauri::command]
pub fn delete_resume_workspace(
    app: tauri::AppHandle,
    workspace_id: String,
) -> Result<ResumeWorkspaceState, String> {
    let app_data_dir = paths::resolve_data_dir(&app)?;
...
pub async fn rewrite_resume_workspace_module(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        rewrite_resume_workspace_module_blocking(app, request)
    })
    .await
...
#[tauri::command]
pub fn assemble_resume_workspace(
    draft: ResumeWorkspaceDraft,
...
pub async fn export_resume_workspace_pdf(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        export_resume_workspace_pdf_blocking(app, draft, title)
    })
    .await
