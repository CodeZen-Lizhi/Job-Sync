...
use super::{
    ...
    shared::{load_job_ai_context, load_job_detail_raw, serialize_job_ai_context, sha256_hex},
...
fn run_resume_analysis(args: ResumeAnalysisArgs) -> Result<Value, String> {
    ...
    let (context_text, resume_files) =
        resolve_resume_sources(saved_settings.as_ref(), context_text, resume_files);
    let resolved_resume_text =
        resume_text::resolve_resume_text(&resume_text, resume_files.as_deref())?;
    ...
    let resume_hash = build_resume_hash(
        &config,
        &resolved_resume_text,
        context_text.as_deref(),
        resume_files.as_deref(),
    );
    let job_context_hash = serialize_job_ai_context(&job_context)?;
    let job_hash = sha256_hex(&format!(
        "job_detail_raw:{job_detail_raw}\njob_ai_context:{job_context_hash}"
    ));
...
fn resolve_resume_sources(
    ...
    let context_text = opt_trimmed(context_text).or_else(|| {
        saved_settings.and_then(|settings| opt_trimmed(Some(settings.ai_context_text.clone())))
    });
    let resume_files = opt_trimmed(resume_files).or_else(|| {
        saved_settings.and_then(|settings| opt_trimmed(Some(settings.ai_resume_files.clone())))
    });
