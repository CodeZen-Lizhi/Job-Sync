...
use super::{
    ...
    shared::{
        attach_job_ai_context, build_group_encrypt_job_id, build_job_outline, describe_job_brief,
        load_job_ai_context, serialize_job_ai_context, sha256_hex, JobAiContext,
    },
...
const MISSING_DETAIL_HEADER: &str =
    "以下职位缺少详情数据，请先采集详情（在职位库展开该职位，或运行自动采集）后再分析：";
...
fn run_profile_analysis(args: ProfileAnalysisArgs) -> Result<Value, String> {
    ...
    let job_contexts = load_job_contexts(&conn, &job_ids)?;
    let profile_hash = build_profile_hash(&config, &context_text);
    let job_hash = sha256_hex_job_details_with_context(&details, &job_contexts)?;

    if !force {
        if let Some(cached) =
            load_cached_report(&conn, &group_encrypt_job_id, &profile_hash, &job_hash)?
        {
            return Ok(cached);
        }
    }
...
fn load_job_contexts(
    conn: &rusqlite::Connection,
    job_ids: &[String],
) -> Result<HashMap<String, JobAiContext>, String> {
    let mut out = HashMap::with_capacity(job_ids.len());
    for job_id in job_ids {
        out.insert(job_id.clone(), load_job_ai_context(conn, job_id)?);
    }
    Ok(out)
}

fn sha256_hex_job_details_with_context(
    details: &[(String, String)],
    contexts: &HashMap<String, JobAiContext>,
    ...
    for (id, raw) in details {
        ...
        if let Some(context) = contexts.get(id) {
            input.push_str(&serialize_job_ai_context(context)?);
        }
        input.push('\n');
    }
    Ok(sha256_hex(&input))
...
fn resolve_context_text(
    ...
    let resolved = opt_trimmed(context_text)
        .or_else(|| {
            saved_settings.and_then(|settings| opt_trimmed(Some(settings.ai_context_text.clone())))
        })
        .unwrap_or_default();
...
fn build_profile_hash(config: &ResolvedOpenAiRequest, context_text: &str) -> String {
    sha256_hex(&format!(
        ...
        config.effective_prompt_extra,
        context_text,
    ))
}

fn build_job_outlines(
    details: &[(String, String)],
    contexts: &HashMap<String, JobAiContext>,
) -> Vec<Value> {
    details
        ...
        .map(|(job_id, raw)| {
            ...
            let mut outline = build_job_outline(job_id, &zp_data);
            if let Some(context) = contexts.get(job_id) {
                attach_job_ai_context(&mut outline, context);
            }
            outline
        })
        .collect()
