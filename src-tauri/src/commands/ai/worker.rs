...
use super::config::{is_local_ollama_base_url, ResolvedOpenAiRequest, OLLAMA_DUMMY_API_KEY};
...
fn apply_openai_overrides(cmd: &mut Command, config: &ResolvedOpenAiRequest) {
    let mut has_api_key_override = false;
    if let Some(api_key) = config.api_key.as_deref() {
        cmd.env("OPENAI_API_KEY", api_key);
        has_api_key_override = true;
    }
    cmd.env("OPENAI_BASE_URL", &config.effective_base_url);
    if !has_api_key_override && is_local_ollama_base_url(&config.effective_base_url) {
        cmd.env("OPENAI_API_KEY", OLLAMA_DUMMY_API_KEY);
    }
    cmd.env("OPENAI_MODEL", &config.effective_model);
    cmd.env("OPENAI_API_MODE", &config.effective_api_mode);
    cmd.env(
        "OPENAI_TEMPERATURE",
...
mod tests {
    ...
    #[test]
    fn openai_effective_config_is_written_to_worker_env() {
        let mut cmd = Command::new("node");
        let config = ResolvedOpenAiRequest {
            api_key: None,
            effective_base_url: "http://localhost:11434/v1".to_string(),
        ...
        assert_eq!(
            env_value(&cmd, "OPENAI_BASE_URL").as_deref(),
            Some("http://localhost:11434/v1")
        );
        assert_eq!(
            env_value(&cmd, "OPENAI_MODEL").as_deref(),
            Some("qwen2.5:7b")
        );
        assert_eq!(
            env_value(&cmd, "OPENAI_API_MODE").as_deref(),
            Some("chat_completions")
        );
        assert_eq!(env_value(&cmd, "OPENAI_API_KEY").as_deref(), Some("ollama"));
    }
    ...
    fn openai_temperature_is_written_to_worker_env() {
        ...
        let config = ResolvedOpenAiRequest {
            ...
            effective_base_url: "https://api.example.test/v1".to_string(),
        ...
        assert_eq!(
            env_value(&cmd, "OPENAI_API_KEY").as_deref(),
            Some("sk-test")
        );
        assert_eq!(
