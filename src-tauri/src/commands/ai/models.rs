...
use super::config::{
    is_local_ollama_base_url, opt_trimmed, resolve_openai_request, OpenAiOverrides,
...
fn resolve_model_list_config(
    ...
    let saved_settings = app_data_dir
        .as_deref()
        .and_then(|dir| settings::read_settings(dir).ok());
    ...
    let base_url = resolved.effective_base_url;

    let api_key = opt_trimmed(request.api_key)
        .or_else(|| {
            saved_settings
                .as_ref()
                .and_then(|settings| opt_trimmed(settings.openai_api_key.clone()))
        })
        .or_else(|| opt_trimmed(std::env::var("OPENAI_API_KEY").ok()))
        .or_else(|| {
            if is_local_ollama_base_url(&base_url) {
                Some(OLLAMA_DUMMY_API_KEY.to_string())
            } else {
                None
            }
        })
