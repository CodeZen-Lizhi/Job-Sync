...
use super::{
    report_meta::{
        compute_ai_report_meta_fields, load_ai_reports, normalize_report_filters,
        persist_ai_report_updates,
    },
    shared::now_rfc3339,
};
...
pub(super) fn upsert_cached_report(
    ...
    let (kind, title, match_score, jobs_count) =
        compute_ai_report_meta_fields(conn, encrypt_job_id, result);
...
pub fn get_ai_report(app: AppHandle, id: i64) -> Result<Value, String> {
    ...
    let result_json: Option<String> = conn
        .query_row(
            "SELECT result_json FROM ai_report WHERE id = ?1",
            [id],
            |row| row.get(0),
        )
        .optional()
