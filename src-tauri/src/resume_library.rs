...
#[derive(Clone, Debug, Deserialize, Serialize)]
pub struct ResumeLibraryOverview {
    pub resumes: Vec<ResumeListItem>,
    pub active_resume_id: Option<String>,
    pub default_resume_id: Option<String>,
    pub selected_resume_id: Option<String>,
    pub linked_jobs: Vec<ResumeJobSummary>,
}
...
impl ResumeLibraryIndex {
    fn normalize(&mut self) {
        ...
        self.job_links
            .retain(|_, resume_id| ids.contains(resume_id));
        if !self
            .default_resume_id
            .as_ref()
...
fn list_item_from_meta(index: &ResumeLibraryIndex, meta: &ResumeMeta) -> ResumeListItem {
    ResumeListItem {
        id: meta.id.clone(),
...
fn linked_jobs_for_resume(
    ...
    let job_ids = index
        ...
        .collect::<Vec<_>>();
    let mut out = job_summaries_for_ids(app_data_dir, index, &job_ids)?;
    out.sort_by(|a, b| {
        a.brand_name
            .cmp(&b.brand_name)
...
fn job_summaries_for_ids(
    app_data_dir: &Path,
    index: &ResumeLibraryIndex,
    encrypt_job_ids: &[String],
) -> Result<Vec<ResumeJobSummary>, String> {
    if encrypt_job_ids.is_empty() {
        return Ok(Vec::new());
    }
    ...
    let sql = format!(
        ...
        WHERE j.encrypt_job_id IN ({placeholders})
        "#
    );
    let mut stmt = conn.prepare(&sql).map_err(|e| e.to_string())?;
    let rows = stmt
        .query_map(params_from_iter(encrypt_job_ids.iter()), |row| {
...
pub fn get_state(
    ...
    let resumes = index
        .resumes
        .iter()
        .map(|meta| list_item_from_meta(&index, meta))
        .collect::<Vec<_>>();
...
pub fn get_overview(
    ...
    Ok(ResumeLibraryOverview {
        resumes,
        active_resume_id: selected_id.clone(),
        default_resume_id: index.default_resume_id,
...
pub fn get_record(app_data_dir: &Path, resume_id: &str) -> Result<Option<ResumeRecord>, String> {
    let mut index = read_index(app_data_dir)?;
    index.normalize();
    let Some(meta) = find_meta(&index, resume_id.trim()) else {
        return Ok(None);
    };
    record_from_meta(app_data_dir, &index, meta).map(Some)
}
...
pub fn delete_resume(app_data_dir: &Path, resume_id: &str) -> Result<ResumeLibraryState, String> {
    ...
    index
        .job_links
        .retain(|_, linked_resume_id| linked_resume_id != resume_id);
...
pub fn get_status_for_job(
    ...
    Ok(status_for_job_from_index(&index, encrypt_job_id))
}

fn status_for_job_from_index(
    index: &ResumeLibraryIndex,
    encrypt_job_id: &str,
) -> ResumeJobLinkStatus {
    let linked_resume_id = index.job_links.get(encrypt_job_id).cloned();
    ...
    ResumeJobLinkStatus {
        ...
        default_resume_id: index.default_resume_id.clone(),
...
pub fn get_statuses_for_jobs(
    ...
    Ok(encrypt_job_ids
        .iter()
        .map(|job_id| status_for_job_from_index(&index, job_id))
        .collect())
}
...
pub fn resolve_resume_text_for_job(
    ...
    let linked_resume_id = index
        ...
        .filter(|id| find_meta(&index, id).is_some())
        .cloned();
    let candidate = linked_resume_id.map(|id| id).or_else(|| {
        index
            .default_resume_id
            .as_ref()
            .filter(|id| find_meta(&index, id).is_some())
    ...
    let title = find_meta(&index, &resume_id)
        .map(|meta| meta.title.clone())
        .unwrap_or_else(|| "简历".to_string());
    let body = read_body(app_data_dir, &resume_id)?;
    if body.trim().is_empty() {
        return Ok(None);
    }
    Ok(Some(ResolvedResume {
        id: resume_id,
        title,
        body,
    }))
...
pub fn resolve_resume_text_by_id(
    app_data_dir: &Path,
    resume_id: &str,
) -> Result<Option<ResolvedResume>, String> {
    let index = read_index(app_data_dir)?;
    let cleaned_id = resume_id.trim();
    let Some(meta) = find_meta(&index, cleaned_id) else {
        return Ok(None);
    };
    let body = read_body(app_data_dir, cleaned_id)?;
    if body.trim().is_empty() {
        return Ok(None);
    }
    Ok(Some(ResolvedResume {
...
pub fn job_summary_for_ids(
    ...
    let index = read_index(app_data_dir)?;
    job_summaries_for_ids(app_data_dir, &index, &encrypt_job_ids)
}
...
mod tests {
    ...
    fn one_job_has_one_resume_and_resume_has_many_jobs() {
        ...
        link_resume_to_jobs(tmp.path(), &second_id, vec!["job-1".to_string()]).expect("overwrite");
    ...
    #[test]
    fn overview_omits_resume_body_and_record_loads_it_on_demand() {
        let tmp = app_data_dir();
        let state = create_resume(tmp.path(), "基础版", "# Large Body").expect("create");
        let resume_id = state.selected_resume.expect("selected").id;

        let overview = get_overview(tmp.path(), Some(&resume_id)).expect("overview");
        assert_eq!(
            overview.selected_resume_id.as_deref(),
            Some(resume_id.as_str())
        );
        let value = serde_json::to_value(&overview).expect("overview json");
        assert!(value.get("selected_resume").is_none());
        assert!(!value.to_string().contains("# Large Body"));

        let record = get_record(tmp.path(), &resume_id)
            .expect("record")
            .expect("selected record");
        assert_eq!(record.body, "# Large Body");
    }
}
