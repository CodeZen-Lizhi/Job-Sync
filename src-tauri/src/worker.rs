...
fn push_bundled_node_candidates(out: &mut Vec<BundledNodeRuntime>, base_dir: PathBuf) {
    ...
    push_node_executable_candidates(out, &base_dir, &working_dir);

    let bin_dir = base_dir.join("bin");
    let working_dir = bin_dir.join("boss-crawler-worker");
    push_node_executable_candidates(out, &bin_dir, &working_dir);
}

fn push_node_executable_candidates(
    out: &mut Vec<BundledNodeRuntime>,
    base_dir: &Path,
    working_dir: &Path,
) {
    out.push(BundledNodeRuntime {
        executable: base_dir.join("node"),
...
pub fn build_worker_command(app: &tauri::AppHandle) -> Result<Command, String> {
    match resolve_worker_program(app) {
        ...
        WorkerProgram::DevNodeScript { entry, working_dir } => {
            build_node_command("node", &entry, &working_dir)
        }
    }
}
