use crate::{
    resume_pdf_support::{
        escape_html, extract_skill_items, file_stem, first_non_empty_line, looks_like_name,
        non_empty_trimmed, parse_entries, ResumeEntry,
    },
...
fn render_profile_item(label: &str, value: &str) -> String {
    let safe = if value.trim().is_empty() {
        "&nbsp;".to_string()
    } else {
        escape_html(value)
    };
...
fn resolve_title(draft: &ResumeWorkspaceDraft, default_title: &str) -> String {
    let from_profile = if draft.basic_profile.name.trim().is_empty() {
        None
    } else {
        Some(draft.basic_profile.name.trim().to_string())
    };
    let from_resume =
        first_non_empty_line(&draft.original_resume_text).filter(|it| looks_like_name(it));
    let from_file = draft
        .original_resume_file
        .as_deref()
        .and_then(file_stem)
        .filter(|it: &String| !it.trim().is_empty());
    from_profile
        .or(from_resume)
        .or(from_file)
...
fn render_summary(content: Option<&str>) -> String {
    let Some(text) = content.and_then(non_empty_trimmed) else {
        return String::new();
    };
    render_section(
        "个人简介",
        format!("<p class=\"summary-text\">{}</p>", escape_html(text)),
    )
}
...
fn render_skills(content: Option<&str>) -> String {
    ...
    render_section(
        "技能清单",
        format!("<div class=\"skills-grid\">{list}</div>"),
    )
}
...
fn render_entry(entry: ResumeEntry) -> String {
    let meta = entry
        .meta
        .as_deref()
        .map(|it| format!("<div class=\"entry-meta\">{}</div>", escape_html(it)))
        .unwrap_or_default();
    let bullets = if entry.bullets.is_empty() {
        ...
        let items = entry
            .bullets
            .iter()
            .map(|it| format!("<li>{}</li>", escape_html(it)))
            .collect::<Vec<_>>()
            .join("");
        format!("<ul class=\"entry-bullets\">{items}</ul>")
    };
    format!(
        "<article class=\"entry\"><div class=\"entry-title\">{}</div>{meta}{bullets}</article>",
