...
use crate::{
    resume_workspace::{self, ResumeBasicProfile, ResumeWorkspaceDraft},
    storage,
};
...
pub struct ResumeWorkspaceMeta {
    ...
    #[serde(default)]
    pub last_exported_pdf_path: Option<String>,
    #[serde(default)]
    pub last_exported_pdf_at: Option<String>,
}
...
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
pub struct ResumeWorkspaceJobStatus {
...
fn legacy_backup_path(app_data_dir: &Path) -> PathBuf {
    storage::storage_dir(app_data_dir).join(format!(
        "resume-workspace.migrated-{}.backup.json",
...
fn create_draft(request: &CreateResumeWorkspaceRequest) -> ResumeWorkspaceDraft {
    ...
    draft.linked_job_id = request.linked_job_id.clone();
    draft
}
...
fn meta_from_draft(
    ...
    ResumeWorkspaceMeta {
        ...
        has_final_resume: draft
            .final_resume_text
            .as_deref()
            .map(str::trim)
            .filter(|it| !it.is_empty())
            .is_some(),
        linked_job_id: draft.linked_job_id.clone(),
        last_exported_pdf_path: draft.last_exported_pdf_path.clone(),
        last_exported_pdf_at: draft.last_exported_pdf_at.clone(),
    }
}
...
fn sync_meta(meta: &mut ResumeWorkspaceMeta, draft: &ResumeWorkspaceDraft) {
    ...
    meta.has_final_resume = draft
        .final_resume_text
        .as_deref()
        .map(str::trim)
        .filter(|it| !it.is_empty())
        .is_some();
    meta.linked_job_id = draft.linked_job_id.clone();
    meta.last_exported_pdf_path = draft.last_exported_pdf_path.clone();
    meta.last_exported_pdf_at = draft.last_exported_pdf_at.clone();
}

fn read_index(app_data_dir: &Path) -> Result<Option<ResumeWorkspaceIndex>, String> {
    let Some(raw) = storage::read_json(&index_path(app_data_dir)).map_err(|e| e.to_string())?
    else {
        return Ok(None);
    };
...
fn read_workspace_draft(
    app_data_dir: &Path,
    workspace_id: &str,
) -> Result<ResumeWorkspaceDraft, String> {
...
fn write_workspace_draft(
    app_data_dir: &Path,
    workspace_id: &str,
    mut draft: ResumeWorkspaceDraft,
) -> Result<ResumeWorkspaceDraft, String> {
    draft.updated_at = Some(now_rfc3339());
    let value: Value = serde_json::to_value(&draft).map_err(|e| e.to_string())?;
    storage::write_json(&workspace_draft_path(app_data_dir, workspace_id), &value)
        .map_err(|e| e.to_string())?;
    Ok(draft)
}

fn workspace_updated_at(workspace: &ResumeWorkspaceMeta, draft: &ResumeWorkspaceDraft) -> String {
    draft
        .updated_at
        .clone()
        .unwrap_or_else(|| workspace.updated_at.clone())
}

fn linked_workspace_drafts_for_job(
    app_data_dir: &Path,
    encrypt_job_id: &str,
) -> Result<Vec<(ResumeWorkspaceMeta, ResumeWorkspaceDraft)>, String> {
    let job_id = encrypt_job_id.trim();
    if job_id.is_empty() {
        return Ok(Vec::new());
    }

    let index = load_or_initialize_index(app_data_dir, false)?;
    let mut matches: Vec<(ResumeWorkspaceMeta, ResumeWorkspaceDraft)> = Vec::new();

    for workspace in index.workspaces {
        let draft = match read_workspace_draft(app_data_dir, &workspace.id) {
            Ok(draft) => draft,
            Err(_) => continue,
        };
        if draft.linked_job_id.as_deref().map(str::trim) != Some(job_id) {
            continue;
        }
        matches.push((workspace, draft));
    }

    matches.sort_by(
        |(left_workspace, left_draft), (right_workspace, right_draft)| {
            workspace_updated_at(right_workspace, right_draft)
                .cmp(&workspace_updated_at(left_workspace, left_draft))
        },
    );
    Ok(matches)
}
...
fn active_workspace_exists(index: &ResumeWorkspaceIndex) -> bool {
    index
        .workspaces
        .iter()
        .any(|workspace| workspace.id == index.active_workspace_id)
}

fn state_from_index(
    app_data_dir: &Path,
    index: ResumeWorkspaceIndex,
...
fn append_workspace(
    ...
    index
        .workspaces
        .push(meta_from_draft(id, title, created_at, &saved_draft));
...
fn load_or_initialize_index(
    app_data_dir: &Path,
    create_blank_if_missing: bool,
) -> Result<ResumeWorkspaceIndex, String> {
    if let Some(index) = read_index(app_data_dir)? {
        if index.workspaces.is_empty() {
            if create_blank_if_missing {
                let mut next = ResumeWorkspaceIndex::default();
                let _ = append_workspace(
                    app_data_dir,
                    &mut next,
...
pub fn delete_workspace(
    ...
    index.active_workspace_id =
        active_fallback_workspace(&index).ok_or("无法确定新的激活工作区。")?;
    write_index(app_data_dir, &index)?;
    state_from_index(app_data_dir, index)
}
...
pub fn get_status_for_job(
    app_data_dir: &Path,
    encrypt_job_id: &str,
) -> Result<Option<ResumeWorkspaceJobStatus>, String> {
    let matches = linked_workspace_drafts_for_job(app_data_dir, encrypt_job_id)?;
    Ok(matches.into_iter().next().map(|(workspace, draft)| {
        let updated_at = workspace_updated_at(&workspace, &draft);
        ResumeWorkspaceJobStatus {
            ...
            has_final_resume: draft
                ...
                .filter(|it| !it.is_empty())
                .is_some(),
            last_exported_pdf_path: draft.last_exported_pdf_path,
            last_exported_pdf_at: draft.last_exported_pdf_at,
        }
    }))
}

pub fn get_final_resume_text_for_job(
    app_data_dir: &Path,
    encrypt_job_id: &str,
    ...
    Ok(matches
        .into_iter()
        .next()
        .and_then(|(_, draft)| draft.final_resume_text)
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty()))
}
