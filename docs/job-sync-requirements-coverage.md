...
| --- | --- | --- | --- | --- |
| Phase 0：源码基线与 Mac Tauri 适配 | Mac Tauri 开发启动、Chrome 路径、sidecar、本机打包边界、命令清单、数据流图 | 部分闭环 | `docs/job-sync-phase0-baseline.md` 已记录架构、命令、Chrome 路径、sidecar、schema、数据流；`README.md` 记录 Mac Chrome 路径、`npm run tauri:dev` 和 `npm run tauri:build:app`；`src-tauri/src/commands/auth.rs` 现在要求 Cookie 和 LocalStorage 都存在才返回已登录，并有 Rust 单测覆盖；`test/review-workflow-contract.test.js` 锁定 release 包装平台感知、app-only 构建入口和旧 target runtime 清理；`packages/boss-crawler-worker/test/sidecar-lifecycle.test.ts` 验证本机 Node worker 生命周期；`src-tauri/tauri.conf.release.json` 配置 macOS ad-hoc 签名；2026-06-14 本机 `npm run tauri:build:app` 已生成 `src-tauri/target/release/bundle/macos/job-sync.app`，并通过 `.app` 签名和包内 worker runtime 生命周期校验。 | 仍需实机确认采集页 UI 启动 sidecar 并渲染事件；`npm run tauri:build` 当前复现失败在 `bundle_dmg.sh`，需修复 DMG 生成与 `hdiutil verify`；真实 Boss 登录后的 Cookie/LocalStorage 持久化复用。 |
...
| Phase 7：Top 20 审核队列 | 展示 Top 20、硬限制结果、简历匹配、偏好、公司风险、沟通状态、上次打招呼时间；审核状态 | 已闭环 | `src/pages/Jobs.vue` 有 Top 20、收藏、准备投递、沟通回溯等队列；`JobsJobItem.vue` 展示筛选画像、评分、公司风险和沟通状态；`useJobsPage.ts` 复制 Top 20 摘要并保持人工确认边界；contract 测试覆盖 Top 20 UI、评分展示、状态操作、恢复候选和无自动投递；`scripts/seed-desktop-smoke-data.mjs` 可生成隔离 SQLite fixture 用于 release app 桌面烟测。 | 仍需实际打开 Tauri UI 截图或操作证明含 fixture 数据的页面渲染正确。 |
...
| 2 | Mac 用户可以通过 Tauri 桌面端完成核心流程 | 部分闭环：Mac release `.app` 已构建并通过签名与包内 worker runtime 校验；DMG 当前失败在 `bundle_dmg.sh`，真实 Tauri 桌面含数据的端到端验收仍需补证据。 |
...
| 18 | 保留简历优化和 PDF 导出，并和筛选岗位联动 | 部分闭环：简历工作区联动、最终稿和 PDF 状态已接入投递材料包；真实 PDF 导出端到端仍需实机样例验证。 |
| 19 | 产品文案围绕精准求职和岗位研究，避免自动投递机器人 | 已闭环。 |

## 下一步优先级

1. **Tauri 实机运行验收**：重新执行 `npm run tauri:dev`，在桌面端验证 Jobs/Crawl 页面能调用 Tauri command，采集页能启动 sidecar 并渲染 worker 事件。
2. **真实数据 UI 烟测**：使用 `npm run seed:desktop:smoke -- <data-dir>` 准备本地 SQLite fixture，验证 Top 20、准备投递、每日情报、打招呼草稿和简历工作区联动在 Tauri 桌面端可用。
