...
<template>
  <div class="ui-panel overflow-hidden transition-colors" :class="expanded ? 'ring-border-strong/70' : ''">
    <button
      type="button"
      class="ui-list-row flex w-full cursor-pointer items-center gap-3 px-4 py-3 text-left"
      :aria-expanded="expanded"
      @click="$emit('toggle-group', group)"
    >
      <svg class="h-4 w-4 shrink-0 text-content-muted transition-transform duration-200" :class="expanded ? 'rotate-90' : ''" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
    ...
    <div v-if="expanded" class="border-t border-border/80">
      ...
      <template v-else>
        <div v-if="!jobs || jobs.length === 0" class="px-6 py-5 text-sm text-content-muted">暂无职位数据。</div>
