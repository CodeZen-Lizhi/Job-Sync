...
const triggerClass = computed(() =>
  isWorkspace.value
    ? "group relative flex min-h-11 w-full items-center justify-between gap-3 rounded-lg border border-border/80 bg-white/95 px-3.5 py-2 text-sm text-content-primary shadow-sm shadow-slate-200/50 transition-colors duration-150 hover:border-border-strong hover:bg-white focus:outline-none disabled:cursor-not-allowed disabled:opacity-60"
...
const optionStateClass = computed(() => ({
  ...
  hover: isWorkspace.value ? "hover:bg-surface-alt" : "hover:bg-surface-alt",
  ...
  active: isWorkspace.value ? "bg-surface-alt text-content-primary" : "bg-surface-alt text-content-primary",
}));
