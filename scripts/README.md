...
- `zhilian-canary.mjs` — interactive Zhilian worker smoke test for the isolated persistent Zhilian profile. Run login mode once to prepare the search-page-readable profile, then collect mode; collect mode passes only after real `JOB_NORMALIZED_CAPTURED` events are emitted.
- `zhilian-db-canary.mjs` — read-only Zhilian App DB canary for a recent real collection run; checks `collection_run`, `job`, `job_source_link`, `job_detail_raw`, and fatal `collection_failure` rows so list-level Zhilian captures are proven to reach the unified job library.
- `npm run zhilian:sidecar-canary` — real Zhilian worker plus Rust sidecar persistence canary for environments where UI clicking is unavailable; set `JOB_SYNC_ZHILIAN_CANARY_PROFILE_DIR` when not using `data/zhilian-browser-profile`. It writes to an isolated temp DB by default; set `JOB_SYNC_ZHILIAN_CANARY_DATA_DIR` only when you intentionally want to verify a specific app data directory with `zhilian:db-canary`.
- `seed-desktop-smoke-data.mjs` — creates an isolated SQLite fixture for desktop UI smoke tests; pass a data directory argument or set `JOB_SYNC_DATA_DIR`, then launch the app with `--data-dir <path>`.
