...
export function buildPostCollectionJudgePrompts(args: {
  ...
  const user = [
    ...
    "5. 语义证据：按岗位原文的实际语义判断，不要求原文逐字复述规则中的概括词。原文已经明确表达等价事实时，应视为有证据；只有事实本身缺失时才按不确定处理。",
    "6. 岗位证据闭包：如果帖子包含多个岗位，必须先选择一个具体岗位作为判断对象；招聘真实性、职责、技术方向、工作方式和其他必须条件都必须来自该岗位，禁止跨岗位拼接证据。",
    `7. 风险与缺失：只有事实缺失、语义含糊或信息矛盾时才参考【AI 风险关注点】，并输出用户设置的不确定策略 ${aiUncertainStrategy}。`,
    "8. recommended：确认是真实招聘，未命中排除条件，并且同一个具体岗位满足用户写明的全部必须条件且整体符合目标。",
    "9. pending_confirmation：仅当用户的不确定策略就是 pending_confirmation 时用于不确定场景。",
    "10. filtered：明确非招聘、明确命中排除条件、明确不满足用户必须条件、无法把必须条件归属于同一个具体岗位，或用户把不确定策略设置为 filtered。",
    "",
    "硬性要求：",
    ...
    "8. recommended 的 evidence 必须覆盖真实招聘证据、具体目标岗位职责，以及用户写明的全部必须条件；多岗位帖子必须在证据中明确岗位名称。",
    "9. 不得使用一个岗位的技术方向、另一个岗位的工作方式或第三个岗位的任职条件共同支持 recommended。",
    "",
    "只输出一个 JSON 对象，字段必须完全符合示例结构：",
    JSON.stringify(schemaHint),
    "",
